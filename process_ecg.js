...
function preprocessECGData(rawData, options = {}) {
  ...
  try {
    ...
    const leads = leadNames.map((leadName, leadIndex) => {
      console.log(
        `Processing lead ${leadIndex + 1}/${leadNames.length}: ${leadName}`
      );
    ...
    const optimizedData = {
      ...
      globalValueRange: {
        min: parseFloat(
          Math.min(...leads.map((lead) => lead.valueRange.min)).toFixed(
            precision
          )
        ),
        max: parseFloat(
          Math.max(...leads.map((lead) => lead.valueRange.max)).toFixed(
            precision
          )
        ),
      },
...
async function main() {
  const inputPath = "./assets/json/10160.json";
  const outputDir = "./priv/static/assets/json/";
  ...
  try {
    ...
    console.log(
      `2. The ECG playback component will automatically detect the preprocessed format`
    );
  } catch (error) {
