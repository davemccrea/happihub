...
class ECGStore {
  ...
  togglePlayback() {
    ...
    if (!newPlayingState) {
      ...
      console.log(
        "⏸️ ECGStore.togglePlayback() paused, recorded pausedTime:",
        this.pausedTime
      );
    } else {
      ...
      if (this.pausedTime && this.startTime) {
        ...
        console.log(
          "▶️ ECGStore.togglePlayback() resuming, adjusted startTime by pauseDuration:",
          pauseDuration
        );
        this.pausedTime = 0;
        ...
        this.pausedTime = 0;
        console.log(
          "▶️ ECGStore.togglePlayback() starting for first time, startTime:",
          this.startTime
        );
      }
    }
  ...
  renderCurrentFrame() {
    ...
    if (
      ...
      if (elapsedSeconds >= this.totalDuration) {
        console.log(
          "🎬 ECGStore.renderCurrentFrame() - elapsed time exceeds duration, returning"
        );
        return;
      }
      ...
      // Use timeout for any canvas recreation to ensure dimensions are fully stabilized
      const timeoutDelay = 1000;
      setTimeout(() => {
        console.log(
          "🎬 ECGStore.renderCurrentFrame() setTimeout callback executing",
          {
            hasRenderer: !!this.renderer,
            hasProcessAnimationFrame: !!(
              this.renderer && this.renderer.processAnimationFrame
            ),
            timeoutDelay,
          }
        );
        ...
        if (this.renderer && this.renderer.processAnimationFrame) {
          // Recalculate cursor progress with the current (potentially updated) widthSeconds
          // This ensures we use the latest canvas dimensions after fullscreen transitions
          ...
          console.error(
            "🎬 ECGStore.renderCurrentFrame() - renderer or processAnimationFrame not available"
          );
        }
      }, timeoutDelay);
    } else {
  ...
  withCanvasStatePreservation(operation) {
    ...
    if (wasPlaying) {
      console.log(
        "🔄 ECGStore.withCanvasStatePreservation() stopping animation temporarily"
      );
      this.isPlaying = false; // Temporarily stop animation
    }

    console.log(
      "🔄 ECGStore.withCanvasStatePreservation() executing operation"
    );
    operation();
    console.log(
      "🔄 ECGStore.withCanvasStatePreservation() operation completed"
    );

    // Restore state
    if (!wasPlaying && this.startTime && this.pausedTime) {
      console.log(
        "🔄 ECGStore.withCanvasStatePreservation() re-rendering current frame for paused state"
      );
      // Re-render current frame for paused state
      this.renderCurrentFrame();
    } else {
      console.log(
        "🔄 ECGStore.withCanvasStatePreservation() not re-rendering frame",
        {
          wasPlaying,
          hasStartTime: !!this.startTime,
          hasPausedTime: !!this.pausedTime,
    ...
    if (wasPlaying) {
      console.log(
        "🔄 ECGStore.withCanvasStatePreservation() resuming animation"
      );
      this.isPlaying = true; // Resume animation
    }
