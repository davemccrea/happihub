...
class UIBinder {
  ...
  updatePlayPauseButton() {
    ...
    if (button) {
      ...
      button.innerHTML = `<svg class="w-4 h-4 ${iconClass}" fill="currentColor" viewBox="0 0 24 24"><path fill-rule="evenodd" d="${
      ...
      } " clip-rule="evenodd" /></svg><span class="ml-1">${buttonText}</span>`;
    }
  }
  ...
  updateFullscreenButton() {
    ...
    if (button) {
      ...
      const tooltip = this.store.isFullscreen
        ? "Exit Fullscreen (f)"
        : "Enter Fullscreen (f)";
      button.innerHTML = `<svg class="w-5 h-5 ${iconClass}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="${
        this.store.isFullscreen
          ? "M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4"
          : "M4 8V4m0 0h4M4 4l4 4m12-4v4m0-4h-4m4 0l-4 4M4 16v4m0 0h4m-4 0l4-4m12 4v-4m0 4h-4m4 0l-4-4"
      }" /></svg>`;
  ...
  setupCheckboxes() {
    this.setupElementListener(
      ...
      (element) => {
        element.checked = this.store.loopEnabled;
      }
    );
    ...
    this.setupElementListener(
      ...
      (element) => {
        element.checked = this.store.qrsIndicatorEnabled;
      }
    );
  }
  ...
  updateAmplitudeScaleDisplay() {
    ...
    if (amplitudeScaleValue)
      amplitudeScaleValue.textContent = `${this.store.amplitudeScale.toFixed(
        2
      )}x`;
